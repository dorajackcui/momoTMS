...
def test_workbench_runs_import_dev_fill_and_qa_jobs() -> None:
    ...
    assert [stage["stage"] for stage in import_job["job"]["summary"]["stages"]] == ["parse", "persist_import"]
    ...
    assert dev_job["job"]["summary"]["stages"][0]["stage"] == "bind_dev_scope"
    ...
    assert [stage["stage"] for stage in fill_job["job"]["summary"]["stages"]] == ["fill_export", "artifact_write"]
    ...
    assert qa_job["job"]["summary"]["stages"][0]["stage"] == "qa_scan"
